 ...
 *
 * SOLAR-ONLY UPDATE:
 *  • We keep using JSON field "deviceName" for backward compatibility
 *    BUT it now represents the Solar Unit name.
 *  • The Solar Unit name is persisted in Preferences under key "solarName".
 *
...
// ✅ Persisted Solar Unit name key (new)
const char* PREF_SOLAR_NAME_KEY = "solarName";
...
struct ControlPacket {
  ...
  float gridPrice;
  char deviceName[24]; // ✅ now Solar Unit name (kept for compatibility)
};
#pragma pack(pop)
...
// ✅ Device info (now Solar Unit name)
...
void onDataRecv(const esp_now_recv_info_t *info, const uint8_t *incomingData, int len) {
  ...
  if (len == (int)sizeof(TelemetryPacket)) {
    ...
    // Ensure currentDevice is always non-empty
...
void initEspNow() {
  ...
  // Re-lock channel after WiFi connection
  esp_wifi_set_promiscuous(true);
  esp_wifi_set_channel(WIFI_CHANNEL, WIFI_SECOND_CHAN_NONE);
  esp_wifi_set_promiscuous(false);
...
void publishTelemetry() {
  ...
  // ✅ "deviceName" kept but now is Solar Unit name
  String json = "{";
  ...
  json += "\"deviceName\":\"" + currentDevice + "\","; // ✅ Solar Unit name
...
void sendTelemetryJson() {
  ...
  if (dataIsFresh) {
    ...
    if (hasTelemetry && (now - lastTelemetryMs) > TELEMETRY_TIMEOUT_MS) dataStatus = "stale";
...
void handle_control() {
  ...
  if (server.hasArg("deviceName")) {
    // ✅ deviceName is now the Solar Unit name
    ...
    // ✅ persist Solar name
    settings.begin("solar_rx", false);
    settings.putString(PREF_SOLAR_NAME_KEY, currentDevice);
    settings.end();
...
void onMqttMessage(char* topic, byte* payload, unsigned int length) {
  ...
  if (!topicStr.endsWith("/control")) return;
    ...
    if (colonIdx >= 0) {
      ...
      if (endIdx > colonIdx) {
        ...
        if (price > 0 && price < 1000) {
          ...
          // keep local mirror consistent
          gridPriceRx = price;
          settings.begin("solar_rx", false);
          settings.putFloat("gridPrice", price);
          settings.end();
        }
      }
    }
      ...
      if (quote1 >= 0) {
        ...
        if (quote2 > quote1) {
          ...
          if (nameStr.length() > 0 && nameStr.length() < 24) {
            ...
            // ✅ Solar-only: apply + persist Solar Unit name
            currentDevice = nameStr;
            settings.begin("solar_rx", false);
            settings.putString(PREF_SOLAR_NAME_KEY, currentDevice);
            settings.end();

            Serial.printf("   Solar Unit name saved: %s\n", cmd.deviceName);
          }
        }
      }
    }
...
void loadSettings() {
  ...
  // ✅ Load persisted Solar Unit name
  currentDevice = settings.getString(PREF_SOLAR_NAME_KEY, "Unknown");
  currentDevice.trim();
  if (currentDevice.length() == 0) currentDevice = "Unknown";
...
void setup() {
  ...
  server.onNotFound([]() {
    server.send(200, "text/html",
      "<!DOCTYPE html><html><head><meta charset=\"UTF-8\"><title>Solar Tracker</title></head>"
      "<body><h1>Solar Tracker Receiver</h1>"
      "<p>WiFi Configuration: <a href=\"/wifi-setup\">/wifi-setup</a></p>"
      "<p>Device running in MQTT mode. Telemetry published to EMQX Cloud.</p>"
      "</body></html>"
    );
  });
    ...
    if (millis() - lastConnectionCheck > 5000) {
      ...
      if (hasTelemetry && lastTelemetryMs > 0) {
        ...
        if (timeSinceLast > CONNECTION_LOST_TIMEOUT_MS) {
          ...
          Serial.println("   - Transmitter off/crashed");
          Serial.println("   - Channel mismatch (should be channel 1)");
