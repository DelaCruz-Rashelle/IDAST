...
GitHub Repository
    ...
    └─→ Vercel Build Pipeline
         ...
         └─ Environment Variables:
            ...
            - NEXT_PUBLIC_MQTT_BROKER_URL, NEXT_PUBLIC_MQTT_USERNAME, NEXT_PUBLIC_MQTT_PASSWORD (optional)
```
...
GitHub Repository (backend/)
    ...
    └─→ Railway Build Pipeline
         ...
         └─ Environment Variables:
            ...
            - MQTT_BROKER_URL, MQTT_USERNAME, MQTT_PASSWORD (for MQTT ingest)
            - INGEST_ENABLED (optional, default true)
```
...
- `GET /api/latest` - Latest device state (energy, battery, timestamp) from `device_state`
- `GET /api/history.csv` - Historical data (CSV) from `device_state` (aggregated by day)
- `GET /api/telemetry` - Query telemetry records from `device_state` (date range, limit)
- `GET /api/device` - Get current device name (latest from `device_registration`)
- `GET /api/devices` - Get all registered devices
- `GET /api/device-stats` - Get device statistics from `device_state` (energy, battery, last seen)
- `POST /api/device` - Save/register a device name (Solar Unit); devices are also auto-registered via MQTT telemetry
- `POST /api/grid-price` - Save grid price and calculate estimated savings (uses `device_state` energy sum)
...
- **Auto-Registration**: Devices are automatically registered when telemetry is received via MQTT (and optionally via `POST /api/device`).
...
**Purpose:** Stores Solar Unit telemetry (energy, battery, timestamp) for metrics and history. Populated by the backend MQTT ingest when telemetry is received on `solar-tracker/+/telemetry`. Used by `/api/latest`, `/api/history.csv`, `/api/telemetry`, `/api/device-stats`, and for grid price estimated savings.
...
**Real-Time Data:**
1. Generated by ESP32 sensors and sent via ESP-NOW to receiver
2. Receiver publishes to MQTT broker (`solar-tracker/{device_id}/telemetry`)
3. Frontend subscribes via MQTT WebSocket for live display
4. Backend MQTT subscriber persists each message into `device_state` and updates `device_registration`

**Historical Data:**
1. Backend: Aggregated from `device_state` (populated by MQTT ingest); served via `/api/history.csv`, `/api/telemetry`, `/api/device-stats`
2. ESP32: Optionally logs to `/history.csv` (LittleFS) every 10 min and can serve via `/api/history`; frontend may use backend APIs for charts

**Database Data:**
1. **device_registration**: Created/updated when MQTT telemetry is received or when user saves device name via `POST /api/device`
2. **device_state**: Inserted on each MQTT telemetry message (energy_wh, battery_pct, ts) for history and metrics
3. **grid_price**: Saved by user via dashboard; estimated savings computed from sum of energy in `device_state`
...
**Limitations:**
- Backend API polling for history/device-stats (e.g. on load or 30s) adds some request volume
- No request batching for API calls

**Current Real-Time Path:**
- Real-time telemetry uses MQTT WebSocket (mqtt.js) to the broker; no HTTP polling for live data.

**Future Improvements:**
- Request batching/aggregation for history and stats
...
1. **ESP32 Receiver ↔ MQTT Broker**: Publishes telemetry; optional Cloudflare tunnel is not required for data path
2. **Frontend ↔ MQTT Broker**: MQTT WebSocket for real-time telemetry
3. **Frontend ↔ Backend**: REST API for device list, history CSV, device-stats, grid price, history-logs
4. **Backend ↔ MQTT Broker**: Subscribes to telemetry; persists to `device_registration` and `device_state`
5. **Backend ↔ Database**: SQL via mysql2
6. **ESP32 ↔ Transmitter**: ESP-NOW (device-to-device)
...
- **Frontend**: Next.js on Vercel (serverless, global CDN); real-time via MQTT WebSocket
- **Backend**: Express.js on Railway (containerized); MQTT subscriber + REST API
- **Database**: MySQL on Railway (managed); `device_registration`, `device_state`, `grid_price`
- **Hardware**: ESP32 receiver (MQTT publisher) + transmitter (ESP-NOW); optional AP-mode WiFi config
- **Broker**: EMQX Cloud MQTT (primary data path); Cloudflare Tunnel optional for legacy/direct ESP32 access
